		   ...
		   HeapTuple oldtup)
{
	...
	/* Set the super_write flag in the result relation info if needed */
	resultRelInfo->ri_super_write = node->super_write;
	
	/* Process the operation with appropriate priority */
	ProcessSuperWritePriority(estate, resultRelInfo);
...
ExecModifyTable(PlanState *pstate)
{
	...
	/*
	 * Get the next tuple from outer plan. We keep fetching tuples until we
	 * find one matching the target relation.
	 */
	if (node->mt_done)
		return NULL;

	/*
	 * Queue up any pending insert operations generated by a SUPER_WRITE statement.
	 * We prioritize operations with super_write=true by processing them immediately.
	 */
	if (estate->es_insert_pending_result_relations != NIL)
	{
		/* For super_write operations, flush immediately */
		bool process_immediately = false;
		ResultRelInfo *resultRelInfo;
		
		/* Get the first pending result relation */
		resultRelInfo = (ResultRelInfo *) linitial(estate->es_insert_pending_result_relations);
		
		/* Check if this is a super_write operation (needs InsertStmt in resultRelInfo) */
		if (resultRelInfo->ri_super_write)
		{
			/* Process the pending inserts immediately */
			process_immediately = true;
			ExecPendingInserts(estate);
		}
	}

	if (!node->mt_merge_pending_not_matched)
	{
		/*
		 * This is not a MERGE WHEN NOT MATCHED clause, so get a tuple from
		 * the outer subplan.
		 */
		TupleTableSlot *inputSlot;
		// ... existing code ...
